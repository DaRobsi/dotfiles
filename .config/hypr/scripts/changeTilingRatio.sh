...
echo $(hyprctl getoption general:layout)

if [[ $(hyprctl getoption general:layout | grep str) == "str: master" ]]; then
    MSG="Layout must be dwindle!"
elif [[ $(echo "$SPLIT_RATIO==1.0" | bc -l) == 1 ]]; then
